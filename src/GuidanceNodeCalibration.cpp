...
geometry_msgs::TransformStamped guidancetransform;
...
int my_callback(int data_type, int data_len, char *content)
{
    ...
    if (e_image == data_type && NULL != content)
    {
        ...
        if ( data->m_greyscale_image_left[CAMERA_ID] ) {
            ...
            try {
              ...
              std::cout<<"Fail to read yaml"<<std::endl;
                  std::cout<<camera_params_left<<std::endl;
        ...
        if ( data->m_disparity_image[CAMERA_ID] ){
            ...
            imshow(string("disparity_")+char('0'+CAMERA_ID), disp8);
        }
    ...
    {
         motion *og=(motion *)content;
         guidancetransform.header.stamp=ros::Time::now();
         guidancetransform.header.frame_id = "world";
         guidancetransform.child_frame_id = "base_link";
...
int main(int argc, char** argv)
{
    ...
    my_node.getParam("/guidanceNodeCalibration/left_param_file", camera_params_left);
    my_node.getParam("/guidanceNodeCalibration/right_param_file", camera_params_right);
